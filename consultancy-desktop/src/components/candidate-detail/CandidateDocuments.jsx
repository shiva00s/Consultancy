...
import { 
    FiFileText, FiCamera, FiTrash2, FiUpload, FiEye, 
    FiDownload, FiAlertTriangle, FiCheckCircle
} from 'react-icons/fi';
...
const documentCategories = [
    'Uncategorized', 'Passport', 'Resume', 'Photograph', 'Education', 
    'Experience Letter', 'Pan', 'Offer Letter', 'Visa', 'Other', // <-- ADDED 'Experience Letter' and 'Pan'
];
...
function CandidateDocuments({ user, candidateId, documents, onDocumentsUpdate }) {
  ...
  const handleDeleteDocument = async (docId, fileName) => {
    if (confirm(`Are you sure you want to move file "${fileName}" to the Recycle Bin?`)) { // Keep native confirm for high-risk actions
  ...
  const handleChangeCategory = async (docId, newCategory, fileName) => {
    const res = await window.electronAPI.updateDocumentCategory({ user, docId, category: newCategory });
    if (res.success) {
  ...
  return (
    <div className="document-tab-content module-vertical-stack">
        ...
        <div className="module-form-card add-docs-section">
          <h3><FiUpload /> Upload More Documents</h3>
          <form className="add-docs-form" onSubmit={(e) => e.preventDefault()}>
            <div className="form-group">
              ...
              <div className="custom-file-input">
                <input 
                  ...
                  ref={fileInputRef}
                />
                <label htmlFor="new-files-input" className="file-input-label btn btn-no-hover">
                  Choose Files
                </label>
            ...
            <button className="btn" onClick={handleAddDocuments} disabled={newFiles.length === 0 || isUploading} style={{gridColumn: '1 / -1'}}>
                {isUploading ? ( 'Uploading...' ) : ( <> Upload {newFiles.length || 0} File(s) </> )}
            </button>
