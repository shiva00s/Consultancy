...
function CandidateVisa({ user, candidateId }) {
  ...
  // ✅ NEW: Fetch auto-fill data from candidate profile and job placements
  const fetchAutoFillData = useCallback(async () => {
    ...
    try {
      ...
      if (jobPlacementsRes.success && jobPlacementsRes.data?.length > 0) {
        ...
        const activeJob = jobPlacementsRes.data.find(j => j.status === 'ASSIGNED') || 
        ...
        <form onSubmit={handleAddVisaEntry} className="visa-form form-grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)', gap: '15px' }}>
          ...
          <div className="form-group">
            ...
            <input 
              ...
              onChange={handleVisaFormChange}
              style={autoFillData?.country ? { backgroundColor: 'rgba(var(--primary-rgb), 0.05)' } : {}}
            />
          </div>
          <div className="form-group">
            ...
            <input 
              ...
              onChange={handleVisaFormChange}
              style={autoFillData?.passport_number ? { backgroundColor: 'rgba(var(--primary-rgb), 0.05)' } : {}}
            />
          </div>
