...
async function checkUserDelegatedAccess(user, featureKey) {
  ...
  if (!ceilingRes.success || !ceilingRes.data[featureKey]) {
    return {
      success: false,
      error: `Access Denied: Global policy has disabled feature "${featureKey}".`,
    };
  }
  ...
  if (user.role === "admin" || user.role === "staff") {
    ...
    return {
      success: false,
      error: `Access Denied: You do not have delegated permission for "${featureKey}".`,
    };
  }
...
async function checkAdminFeatureAccess(user, featureKey) {
  if (featureKey === "canViewReports" || featureKey === "canAccessSettings") {
    // For these 'global' permissions, Admins rely on the ceiling only,
    // but Staff relies on delegation. Use the main check.
    return checkUserDelegatedAccess(user, featureKey);
  }
  // For all other CRUD features, use the full delegation check.
  return checkUserDelegatedAccess(user, featureKey);
}
...
async function addUser(username, password, role) {
  ...
  if (callingUser.role === "admin") {
    ...
    if (role !== "staff") {
      return {
        success: false,
        error: "Access Denied: Admins can only create Staff accounts.",
      };
    }
    ...
    return {
      success: false,
      error: "Access Denied: Staff cannot add new users.",
    };
  }
  // --- Validation ---
  const errors = {};
...
async function getReportingData(user, filters = {}) {
  ...
  try {
    ...
    // 🐞 FIX: Use dbGet for single aggregate result (replaces openJobsRows/runQuery)
...
async function getSystemAuditLog(user, params) {
  ...
  try {
    ...
    // Correctly concatenate limit/offset
...
async function updateCandidateText(id, data) {
  ...
  // --- 1. CONSISTENT DATA CLEANING (MANDATORY FIX) ---
  const cleanPassportNo = data.passportNo
    ? data.passportNo
        .trim()
        .replace(/[^A-Z0-9]/gi, "")
  ...
  // --- 2. BASIC & REQUIRED VALIDATION (Root Cause Check) ---
  ...
  // Passport No is required (Using the cleaned value)
  ...
  if (data.passportExpiry) {
    const expiryDate = new Date(data.passportExpiry).getTime();
    // Check if date parsing failed (isNaN) or if it's already expired
  ...
  if (data.aadhar) {
    ...
    // else if (validateVerhoeff && !validateVerhoeff(data.aadhar)) {
    //    errors.aadhar = 'Invalid Aadhaar Number (Checksum failed). Please check for typos.';
    // }
  }

  // --- 4. Validation Failure Check ---
  if (Object.keys(errors).length > 0) {
  ...
  try {
    let checkSql =
      "SELECT passportNo, aadhar FROM candidates WHERE (passportNo = ?";
    // Use the cleaned value for the check
    ...
    // Exclude the current candidate by ID
    ...
    const updateParams = [
      ...
      data.experience,
      data.dob,
      cleanPassportNo, // FIX: Use cleanPassportNo here!
      data.passportExpiry,
      data.contact,
      data.aadhar,
      data.status,
...
async function addVisaEntry(data) {
  ...
  try {
    ...
    return {
      success: false,
      error: err.message || "Database execution failed during INSERT.",
    };
  }
}
...
async function updateVisaEntry(id, data) {
  ...
  try {
    ...
    if (result.changes === 0) {
      return {
        success: false,
        error: "Visa entry not found or already deleted.",
      };
    }
...
async function updateMedicalEntry(id, data) {
  ...
  try {
    ...
    if (result.changes === 0) {
      return {
        success: false,
        error: "Medical entry not found or already deleted.",
      };
    }
    const updatedRow = await dbGet(
      db,
...
async function updateTravelEntry(id, data) {
  ...
  try {
    ...
    if (result.changes === 0) {
      return {
        success: false,
        error: "Travel entry not found or already deleted.",
      };
    }
    const updatedRow = await dbGet(
      db,
...
async function updateInterviewEntry(id, data) {
  ...
  try {
    ...
    if (result.changes === 0) {
      return {
        success: false,
        error: "Interview entry not found or already deleted.",
      };
    }
...
async function updatePassportEntry(id, data) {
  ...
  try {
    ...
    if (result.changes === 0) {
      return {
        success: false,
        error: "Passport entry not found or already deleted.",
      };
    }
    const row = await dbGet(
      db,
...
async function deletePassportEntry(id) {
  ...
  try {
    ...
    return { success: true, candidateId: row.candidate_id }; // Return candidate ID for logging
  } catch (err) {
    return { success: false, error: err.message };
  }
}
...
const proxyRequest = async (
  ...
  try {
    ...
    return {
      success: false,
      error: error.response?.data?.error || "Could not connect to remote API.",
    };
  }
};
...
async function permanentDeletePlacement(id) {
  ...
  return {
    ...
    error: result.changes ? null : "Placement not found.",
  };
}
