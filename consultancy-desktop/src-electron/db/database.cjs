...
const passportColsToAdd = [
          { name: 'source_type', type: 'TEXT DEFAULT "Direct Candidate"' },
          { name: 'agent_contact', type: 'TEXT' },
          { name: 'received_notes', type: 'TEXT' },
          { name: 'dispatch_notes', type: 'TEXT' },
          { name: 'docket_number', type: 'TEXT' }
      ];

      passportColsToAdd.forEach(col => {
          dbInstance.run(`ALTER TABLE passport_tracking ADD COLUMN ${col.name} ${col.type}`, (err) => {
              // Ignore "duplicate column" errors if they already exist
              if (err && !err.message.includes('duplicate column name')) {
                   console.error(`Error adding ${col.name} to passport_tracking:`, err.message);
              }
          });
      });
